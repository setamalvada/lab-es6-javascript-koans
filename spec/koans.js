...
describe('`let` restricts the scope of the variable to the current block - ', () => {
  describe('`let` vs. `var`.', () => {
    //con let fuera funciona, pero si lo metes dentro no
    ...
    //let se mete fuera de la función
    ...
    it('`var` does not restricts scope to inside the block in `for` loops', () => {
      //var se puede volver a asignar en un loop, pero let no
      var counter = 100
...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      // const no se puede re-asignar con numeros
       const constNum = 0;
       //constNum = 1;
  ...
  describe('complex types are NOT fully read-only', () => {
    ...
    it('arrays is not fully read-only', () => {
      //una constante simple no se puede reasignar, pero un array u objeto si
      const arr = [42, 23];
      arr[0] = 0
...
describe('`string.includes()` finds string within another string. ', () => {
  ...
  describe('find a string', function() {
    it('that matches exactly', function() {
       const findSome = o => 'xyz'.includes(o);
  ...
  describe('search for an empty string, is always true', function() {
    it('in an empty string', function() {
      //si es un string vacío será true
      const x = ""
